              ...
              <div class="col-8">
                <div class="card-body">
                  ...
                  <ul class="yd-card_info">
                    ...
                    <li v-if="item.price_range || item.price">
          ...
          <div class="yd-reviews">
            ...
            <form class="yd-review-form" @submit.prevent="">
              ...
              <label class="yd-comment">
                ...
                <button class="btn btn-primary" @click="onReviewClick">
                  {{ $t("send") }}
                </button>
              </label>
            </form>
...
export default {
  name: "TheClinicDetails",
  async created() {
    await this.$nuxt.$on("toggleClinicDetailModal", async (item) => {
      this.item = item;
      this.rate = 0;
      ...
      this.$bvModal.show("ClinicDetailsModal");
    });
  },
  data() {
    return {
  ...
  methods: {
    ...
    async setFeedback() {
      let reviews;
      try {
        let cData = { id: this.item.id, type: "clinic" };
    ...
    closeModal() {
      this.$bvModal.hide("ClinicDetailsModal");
      this.$nuxt.refresh();
    },
    setCustomTrack(action) {
      let data = {
        clinic: this.item.id,
        service: (this.item.serviceInfo || {}).id,
        action,
      };
      if (this.item.origin == "analyze") {
        data.selector = "ANALYZE";
      } else if (this.item.origin == "service") {
        data.selector = "SERVICE";
      } else if (this.item.origin == "unit doctor") {
        data = {
          clinic: this.item.id,
