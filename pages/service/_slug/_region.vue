<template>
  <main>
    <div class="yd-page-header">
      <p class="title">{{ $t("search_service") }}</p>
      ...
      <div class="container">
        ...
        <nav class="yd-breadcrumb" aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" v-b-modal.ProcedureModal>
              <a href="#">{{ $t("procedures") }}</a>
            </li>
        ...
        <h1 class="yd-page-title">
          ...
          {{
            getRegionLocalText(
              getRegionIdBySlug(regionSlug(), regionsList) || null
            )
          }}
...
export default {
  ...
  async asyncData({ app, store, params, query, error, $config }) {
    ...
    let itemInfo = store.getters["getServiceBySlug"](params.slug);
    ...
    let clinicsRef = clinicsRes.services.filter(
      (x) => x.service_id == itemInfo.id
    );
    // let clinicsRef = clinicsRes.services.filter((x) => x);
  ...
  data() {
    return {
      service() {
        ...
        let service = this.$store.getters["getServiceBySlug"](slug);
  ...
  head() {
    return {
      title: this.getSeoTitle(
        this.service(),
        this.getRegionIdBySlug(this.regionSlug(), this.regionsList) || null
      ),
      meta: [
        {
          hid: "description",
          name: "description",
          content: this.getSeoDescription(
            this.service(),
            this.getRegionIdBySlug(this.regionSlug(), this.regionsList) || null
          ),
        },
